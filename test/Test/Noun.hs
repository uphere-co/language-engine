...
import           NLP.Syntax.Type.XBar            (TaggedLemma,DetP,AdjunctDP(..)
                                                 ,adjunct,headX,maximalProjection,specifier
...
type TestNoun = (Text
                ,(Text,[Text],[Text])  -- ^ (head,spec,adjunct)
                ,[(Int,(Lemma,Text))]
...
checkBNM x =
  ...
  in (headText tagged dp == x^._2._1)
     &&
     ((dp^..specifier.traverse.to (T.intercalate " " . tokensByRange tagged)) == (x^._2._2))     
     &&
     ((dp^..adjunct.traverse.to (T.intercalate " " . tokensByRange tagged . adjunctDPToRange)) == (x^._2._3))

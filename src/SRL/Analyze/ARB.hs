  ...
  (frmtxt,msense,mneg) <- case v of
                            ...
                            MGPredicate        {..} ->
                              case _mv_pred_info of
                                PredVerb _ sns _ vrb -> return (unFNFrame _mv_frame,sns,vrb^.vp_negation)
                                PredPrep _       -> return (unFNFrame _mv_frame,Nothing,Nothing)
                                PredNominalized _ _ _ -> return (unFNFrame _mv_frame,Nothing,Nothing)
...
mkARB rolemap mg = filter (^.subjectA._1.to (not.T.null)) . catMaybes $ do
